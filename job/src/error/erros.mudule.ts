...
export class ExternalServerError extends Error {
  ...
  constructor(context: string, error?: Error | unknown) {
    ...
    this.status = 503;
    this.message = "Ocorreu um erro ao consultar um serviço externo, tente novamente mais tarde.";
    this.context = context;
...
export function HandleInstanceError(error: unknown) {
  ...
  if (listErrosInternal.some((acceptedError) => error instanceof acceptedError)) {

    const response = {
      status: error instanceof AxiosRequestServiceError? 503 : (error as any).status,
      message: (error as any).message,
    };
